...
class Rep2Graph:
    ...
    def read_report(self, report_file_path):
        ...
        with open(report_file_path) as json_file:
            ...
            if 'behavior' in data.keys():
                ...
                log(self.logfile, '\t No behavior tag found.')
                return None


    def r2g(self, report_file_path, label, log_dir=None, report_cut_out_dir=None, nx_png_out_dir=None, dot_out_dir=None, render_svg=False):
        """
        ...
        if behavior is not None:
            log(self.logfile, '[Rep2Graph][r2g] Processing...')
            ...
            log(self.logfile, '[Rep2Graph][r2g] Behavior none. Skip')
            return False
    ...
    def get_node_name(self, node):
        ...
        node_identifier_str = n_name+'_'+n_type

        return n_name, node_identifier_str
    

    def check_precise_ntype(self, n_type, c_type, handle_addr):
        """
        With the handle node, we need to know more precisely what type of handle is that (key_handle ~ registry, file_handle ~ file, or others) 
        """

        if c_type in ['file_handle', 'key_handle']:
            """ If `c_type` is `file_handle` or `key_handle`, use `c_type` value to set for this node's `type` (instead of purely `handle`). 
                At the same time, check all the previous nodes of type `handle`, to see if the handle address is there.
                If it's there, change the type of these node to `c_type`.
                Also change the edge details accordingly """

            # log(self.logfile, 'handle', handle_addr, c_type)

            n_type = c_type

            """ find idx of the handle node with this address """
            nod_iden_str = None
            nod_idx = None
            for n_iden_str, node in self.map_nodestr_node_by_type['handle'].items():
                hdl_addr = node['address']
                if hdl_addr == handle_addr: #? found a handle node that has same address
                    nod_iden_str = n_iden_str
                    nod_idx = node['id']
                    #? it's safe to break when found. because we dont allow same node being added to the graph twice. so if found, it's the only one.
                    break

            #? no handle node with this address existed in graph. out function
            if nod_iden_str is None:
                return

            """ after break, nod_idx will have value of where iteration stops
                change the type of existing handle node by delete the node in the old ntype dict, and move to self.map_nodestr_node_by_type[n_type] """
            self.map_nodestr_node_by_type[n_type][nod_iden_str] = self.map_nodestr_node_by_type['handle'][nod_iden_str]
            #? remember to delete this node from old types
            del self.map_nodestr_node_by_type['handle'][nod_iden_str]

            #? change graphviz dict for graphviz visualization
            self.g_codes['nodes'][nod_idx] = self.g_codes['nodes'][nod_idx].replace('_handle', f'_{n_type}')
            #? change networkx node details to change node color when visualizing
            self.Gx_nodes[nod_idx]['type'] = n_type

            """ change the edge type also """
            for et in ['api-handle', 'handle-api']:
                #? ek: edge_identifier_str | ed: edge
                for ek in list(self.map_edgestr_edge_by_type[et].keys()):
                    ed = self.map_edgestr_edge_by_type[et][ek]
                    log(self.logfile, f'ek,ed = {ek}, {ed}  | nod_idx = {nod_idx}')
                    #? because ek = {s_node_idx}--{d_node_idx}, if ek not contains nod_idx, its not the edge between the handle node we're looking for
                    if str(nod_idx) not in ek:
                        continue
                    
                    #? found an edge between the handle node of this address to some node. change the edge type accordingly
                    new_et = et.replace('handle', n_type)
                    # log(self.logfile, f'  change edge type from {et} to {new_et}')
                    #? insert this edge to the edges list of new type
                    self.map_edgestr_edge_by_type[new_et][ek] = ed
                    #? delete this edge from the edges list of old type
                    del self.map_edgestr_edge_by_type[et][ek]

                    #? change graphviz dict for graphviz visualization
                    # log(self.logfile, ' >>', self.g_codes['edges'][ek])
                    self.g_codes['edges'][ek] = self.g_codes['edges'][ek].replace(f'[color="{self.map_etype2style[et]}"]', f'[color="{self.map_etype2style[new_et]}"]')
        
        elif c_type in ['handle', 'base_handle']:
            """ Sometimes, key is just handle, but it's still a key_handle. 
                We need to check that. By comparing with previous nodes of type `key_handle` or `file_handle`, to see if the address is there. 
                If it's there, change n_type accordingly """

            # log(self.logfile, 'handle', handle_addr, c_type)
            for nt in ['file_handle', 'key_handle']:
                for nod_iden_str, node in self.map_nodestr_node_by_type[nt].items():
                    if node['address'] == handle_addr:
                        n_type = nt
    ...
    def insert_node(self, node):
        ...
        """ Get more precised n_type
            First thing, with the handle node, we need to know more precisely what type of handle is that (key_handle ~ registry, file_handle ~ file, or others) """
        if n_type == 'handle':
            #? with handle node, node name is the handle address
            n_type = self.check_precise_ntype(n_type, node['ctype'], node['name'])
            # log(self.logfile, f'\n {node} {n_type}')
            node['type'] = n_type
        ...
        if node_identifier_str not in self.map_nodestr_nodetype.keys():
            ...
            return node
        print('>>> n_type', n_type, node)
        print('>>> self.map_nodestr_nodetype[node_identifier_str]', self.map_nodestr_nodetype[node_identifier_str])
        print('>>> self.map_nodestr_node_by_type[n_type]', self.map_nodestr_node_by_type[n_type])
    ...
    def remove_node(self, node):
        ...
        #* DEBUG
        # log(self.logfile, f'self.g_codes[-3] = {self.g_codes[-3]}')
        # log(self.logfile, f'self.g_codes[-2] = {self.g_codes[-2]}')
        # log(self.logfile, f'self.g_codes[-1] = {self.g_codes[-1]}')
        # log(self.logfile, f'self.g_nodes[-1] = {self.g_nodes[-1]}')
        # log(self.logfile, f'self.g_edges[-1] = {self.g_edges[-1]}')
    ...
    def insert_edge(self, source_node, dest_node, api_args=None, buffer_size=None,e_type=None) -> None:
        ...
        if s_node_idx == d_node_idx:
            log(self.logfile, f'[-] Do not allow self loop  {source_node}')
            return
        ...
        if source_node['type'] not in self.n_types or dest_node['type'] not in self.n_types:
            log(self.logfile, '[-] source_node type or dest_node type out of concern: {}, {}'.format(source_node['type'], dest_node['type']))
            return
        
        if self.do_limit_edge is True and e_type not in self.limit_etypes:
            log(self.logfile, '[-] edge type out of concern: {e_type}')
            return
    ...
    def encode_report(self, behavior, report_name, label, log_dir, report_cut_out_dir, nx_png_out_dir, dot_out_dir, render_svg) -> bool:
        ...
        for proc in procs:
            ...
            if n_valid_api == 0:
                log(self.logfile, f'[-] Proc {proc_pid} has no connection. Skip')
                continue
            ...
            log(self.logfile, f'    n_api_calls = {len(api_calls)}   |   n_valid_api: {n_valid_api}')


            # """
            # Before looping through all apis, sometimes a handle is mentioned only once. 
            # This does not make any sense. Especially when the API mentions it is the NtClose. 
            # Quickest way to remove this situation to skip processing is to change the handle address of these circumstances to 0x000000000
            # """
            # handle_occurences = {
            #     'n': 0,
            #     'last_api': None
            ...
            #     if 'last_error' in api and api['last_error'] > 0:
            #         continue
            #     for arg_key in api_args:
            #         if 'handle' in arg_key:
            #             hd_addr = api_args[arg_key]
            #             handle_occurences[hd_addr] = {
            #                 'n': handle_occurences[hd_addr]['n'] + 1,
            #                 'last_api': api['api']
            #             }
            ...
            for api_call in api_calls:
                ...
                if api_call_stringify == last_api_call_stringify:
                    log(self.logfile, f'[-] Duplicate api. Skip.')
                    continue
                ...
                if 'last_error' in api_call and api_call['last_error'] > 0:
                    # log(self.logfile, f'\t API returning error. Skip.')
                    continue
                ...
                # log(self.logfile, f'\t api_name: {api_name} | function_name: {func_name} | last_api_name: {last_api_name}')
                ...
                if api_name_orig == 'LdrGetProcedureAddress':
                    ...
                    # log(self.logfile, f'\t api_args: {api_args}')
                ...
                if self.do_limit_cat is True and api_cat not in self.limit_cats:
                    log(self.logfile, f'[-] Out of limit_cats ({api_cat}). Skip.')
                    continue
                
                node_api_attr = self.insert_node(node_api_attr)
                # last_api_name = api_name

                #? If t's error when inserting node, skip all the rest process
                if node_api_attr is None:
                    log(self.logfile, f'[-] node_api_attr = None. Skip.')
                    continue
                ...
                """
                #* DEBUG
                log(self.logfile, f'    node         : {node_api_attr}')
                log(self.logfile, f'      has_module : {has_module}')
                log(self.logfile, f'      has_handle : {has_handle}')
                log(self.logfile, f'      has_proc   : {has_proc}')
                if (has_module or has_handle or has_proc) is False:
                    log(self.logfile, '[!] api has no connection to any module/handle/proc. Remove this api node')
                    self.remove_node(node_api_attr)
                else:
                    """ Draw an edge from proc node to this api """
                    # log(self.logfile, f'    Draw edge from `node_proc_attr` -> `node_api_attr`   |   {node_proc_attr}   ->   {node_api_attr}')
    ...
    def stats(self, report_name) -> None:
        ...
        """
        log(self.logfile, f'\n--------\n[ ] Stat {report_name}')
        # log(self.logfile, 'self.map_nodestr_node_by_type : {self.map_nodestr_node_by_type}')
        # log(self.logfile, 'Gx_nodes : {self.Gx_nodes}')
        # log(self.logfile, 'Total process : {}'.format(len(self.map_nodestr_node_by_type['proc'])))
        # log(self.logfile, '      API     : {}'.format(len(self.map_nodestr_node_by_type['api'])))
        # log(self.logfile, '      handle  : {}'.format(len(self.map_nodestr_node_by_type['handle'])))
        # log(self.logfile, '      module  : {}'.format(len(self.map_nodestr_node_by_type['module'])))
        # for n_type in ['handle', 'module']:
        for n_type in self.n_types:
            log(self.logfile, '    Total {} \t: {}'.format(n_type, len(self.map_nodestr_node_by_type[n_type])))
        ...
        for n_type in self.n_types:
            for node_identifier_str, node in self.map_nodestr_node_by_type[n_type].items():
                if node['id'] in node_no_edge_idxs:
                    ...
                    break
        log(self.logfile, f'    > node_no_edge_idxs  : {node_no_edge_idxs}')
        log(self.logfile, f'    > node_no_edge_types : {node_no_edge_types}')
    ...
    def visualize_graphviz(self, report_name, dot_out_dir, render_svg) -> None:
        ...
        if dot_out_dir is not None:
            ...
            dot_temp += '\n}'
            # log(self.logfile, f'\t dot_temp: {dot_temp}')
        ...
        if nx_png_out_dir is not None:
            ...
            for n_type in self.n_types:
                if len(self.map_nodestr_node_by_type[n_type]) == 0:
                    continue
                nodes_lbls = [node['id'] for node in self.map_nodestr_node_by_type[n_type]]
    ...
    def processModuleArgs(self, api_name_orig, node_api_attr, api_args):
        ...
        if api_name_orig == 'LdrLoadDll':
            ...
            log(self.logfile, f'[-] `api_name_orig` = {api_name_orig} out of [LdrLoadDll, LdrGetDllHandle, LdrGetProcedureAddress, LdrUnloadDll]')
            return False
        

        #? we only consider some modules. With other unimportant ones, just skip the rest process
        if module_name in ['uxtheme', 'xmllite']:
            log(self.logfile, f'[-] Unimportant module {module_name}. Skip')
            return False
    ...
    def processHandleArgs(self, api_args, node_api_attr):
        ...
        if 'NtClose' in node_api_attr['name']:
            if 'handle' in api_args:
                ...
                for nt in ['handle', 'file_handle', 'key_handle']:
                    for nod_iden_str, nod in self.map_nodestr_node_by_type['handle'].items():
                        if nod['address'] == handle_addr:
                            #? this handle already existed
                            exists = True
                if exists is False:
                    return False
            else:
                log(self.logfile, f'[!] !!!!! NtClose without handle.  node_api_attr = {node_api_attr}  |  api_args = {api_args}')

        """ loop through all arguments """
        for arg_key in api_args:
            #? check if any argument is handle, any kind of handle (section_handle, key_handle, base_handle,...)
    ...
    def processProcArgs(self, api_args, node_api_attr, proc, node_proc_attr):
        ...
        if api_args['process_identifier'] != proc['pid']:
            ...
            log(self.logfile, f'    [processProcArgs] node_nproc_attr = {node_nproc_attr}')
